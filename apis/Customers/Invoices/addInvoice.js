                ...
                staffMemberModel.findById(bookedBy, (error, result) => {
                    if (error) {
                        ...
                        if (result === null) {
                            res.json({
                                data: result,
                                message: "Check Booked By staff Id"
                            })
                        } else {
                            // res.send(result)
                            ...
                            staffMemberModel.findById(deliveredBy, (error, result) => {
                                if (error) {
                                    ...
                                    if (result === null) {
                                        res.json({
                                            data: result,
                                            message: "Check Delivered By staff Id"
                                        })
                                    } else {
                                        // res.send(result)
                                        ...
                                        customerModel.findById(customerId, (error, result) => {
                                            if (error) {
                                                ...
                                                if (result === null) {
                                                    res.json({
                                                        data: result,
                                                        message: "Check Customer Id in supply order"
                                                    })
                                                } else {
                                                    // res.send(result)
                                                    ...
                                                    if (CalculateTax === undefined || CalculateTax.length === 0) {
                                                        ...
                                                        const Invoice = new InvoiceModel({
                                                            ...
                                                            invoiceDate: moment(req.body.invoiceDate).format("DD/MM/YYYY"),
                                                            dueDate: moment(req.body.dueDate).format("DD/MM/YYYY"),
